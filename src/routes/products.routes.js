...
routes.post("/products",upload.single("thumbnail"),sessionAuth,roleAuth(["admin","premium"]), productsModos.addProduct);
...
routes.all('*', async (req, res,next) => {next( new CustomError(errorDicctionary.ROUTING_ERROR))});
